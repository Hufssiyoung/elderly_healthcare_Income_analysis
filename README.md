...
| **Track C** | 군집화 (Unsupervised) | 지역 특성별 군집화 | K-Means |
...
| 서울시 국민기초생활 수급자 동별 현황 (2024) | 행정동 | Track B D4 65세이상 기초수급 비율 |
| 지역사회건강통계(질병이환) | 자치구 | Track B D3 미충족의료율 |
| 2023년 12월 시군구별 등급판정현황 | 시군구 | Track B D1 노인장기요양 수급자 비율 |

---

## 최종 피처 (X 변수 10개 — df_analysis.csv)


| 변수 | 역할 | VIF | r(Y) | 유형 |
|------|------|-----|------|-----|
| 고령화율 | 현재 고령화 수준 | **1.95** | −0.02 | 인구통계 |
| 고령화율_변화 | 고령화 진행 속도 (2022→2023) | **1.73** | +0.24 | 트렌드 |
| 독거노인비율 | 독거 취약도 | **1.38** | **+0.35** | 인구통계 |
| 비율_75-79세 | 수요 급증 전환 구간 | **1.32** | −0.29 | 연령구성 |
| 비율_80-84세 | 고수요 전단계 구간 | **<3** | −0.33 | 연령구성 |
| 비율_85세이상 | 최고령 고수요층 | **1.16** | −0.21 | 연령구성 |
| 인프라갭(65세이상인구 / (의료기관수+1)) | 의료기관 접근성 부족도 | **1.54** | +0.01 | 인프라 |
| 포화도(요양시설 현원/정원) | 요양시설 포화 여부 | **1.65** | −0.17 | 인프라 |
| 시설수 | 요양시설 절대 공급량 | **2.53** | +0.16 | 인프라 |
| 수요공급갭지수 | NHIS 가중 미충족 수요 | **2.33** | −0.19 | 수요대비공급부족 |
...
| 도메인 | 가중치 | 지표 | 단위 |
|--------|--------|------|------|
| D1 노인 의료 필요도 | 30% | 장기요양 등급판정 수급자수 / 65세이상인구 | 시군구→행정동 |
| D2 서비스 접근성 | 25% | 장기요양기관 부족도 (−기관수/천명) | 행정동 (법정동→행정동 매핑) |
| D3 의료 접근성 | 25% | 연간 미충족의료율(%) | 자치구→행정동 |
| D4 소득박탈 | 20% | 65세이상 기초생활수급자수 / 65세이상인구 | 행정동 |
...
- **결측치 처리**: 의료기관수 NaN → 0 / 나머지 10개 피처 → KNN Imputer (n_neighbors=5)
